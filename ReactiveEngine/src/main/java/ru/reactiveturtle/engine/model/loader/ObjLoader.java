package ru.reactiveturtle.engine.model.loader;
...
public class ObjLoader {
    public static Model load(String path, float textureX, float textureY) throws IOException {
            ...
            if (!line.startsWith("#")) {
                ...
                switch (param[0]) {
                    ...
                    case "usemtl":
                        if (materialKey != null) {
                            int counter = 0;
                            String objectKey = materialKey;
                            boolean isUnique = false;
                            while (!isUnique) {
                                isUnique = true;
                                for (Mesh mesh : meshes) {
                                    if (mesh.getKey().equals(objectKey)) {
                                        counter++;
                                        objectKey = materialKey + counter;
                                        isUnique = false;
                                    }
                                }
                            }
                            Object[] vert = cut(vertices, indices, normals, newNormalIndices, texCoords, newTexCoordIndices);
                            Mesh mesh = new Mesh(objectKey, (float[]) vert[0], (int[]) vert[1]);
        ...
        System.out.println("Объект загружен. Время: " + (System.currentTimeMillis() - startTime) + " миллисекунд");
