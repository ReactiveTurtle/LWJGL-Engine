...
import ru.reactiveturtle.game.engine.base.control.CursorCallback;
import ru.reactiveturtle.game.engine.camera.PerspectiveCamera;
import ru.reactiveturtle.game.engine.light.Light;
import ru.reactiveturtle.game.engine.shadow.ShadowManager;
        ...
        while (!glfwWindowShouldClose(defaultWindow)) {
            if (isWindowFocused) {
                ...
                if (glfwGetTime() - time >= 0.2f) {
                    float rotationX = camera.getRotationX();
                    float rotationY = camera.getRotationY();
                    float rotationZ = camera.getRotationZ();
                    glfwSetWindowTitle(defaultWindow, NAME + "(" + (int) (1d / deltaTime) + ") Позиция: "
                            + camera.getX() + ", "
                            + camera.getY() + ", "
                            + camera.getZ()
                            + "; Поворот: "
